...
import type {
  GetTrackResponse,
  UpdateTrackPublicStatusRequest,
  ...
  return useMutation({
    ...
    onSuccess: async (_, { id, isPublic }) => {
      queryClient.setQueryData(
        ['tracks', id],
        (old: GetTrackResponse | undefined) => ({ ...old, isPublic: isPublic })
      )
...
export function useFlushUpdateTrackVisibility(): FlushFn<
  UpdateTrackVisibilityMutation['payload']
> {
  const api = useApi()
